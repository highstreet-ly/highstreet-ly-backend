...
{
    public class PlanRepository : EntityFrameworkCoreRepository<Plan, Guid>
    {
        private readonly IHttpContextAccessor _httpContextAccessor;
        public PlanRepository(ITargetedFields targetedFields, IDbContextResolver contextResolver, IResourceGraph resourceGraph, IResourceFactory resourceFactory, IEnumerable<IQueryConstraintProvider> constraintProviders, ILoggerFactory loggerFactory, IHttpContextAccessor httpContextAccessor) : base(targetedFields, contextResolver, resourceGraph, resourceFactory, constraintProviders, loggerFactory)
        {
            _httpContextAccessor = httpContextAccessor;
        }

        public override Task UpdateAsync(Plan resourceFromRequest, Plan resourceFromDatabase, CancellationToken cancellationToken)
        {
        ...
        public override Task CreateAsync(Plan resourceFromRequest, Plan resourceForDatabase, CancellationToken cancellationToken)
        {
            if (_httpContextAccessor
                .HttpContext == null)
            {
